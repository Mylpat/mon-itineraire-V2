...
export default function SavedItineraries({ itineraries, onView, onDelete }: SavedItinerariesProps): React.ReactElement {
  ...
  return (
    <div className="mt-8 p-6 sm:p-8 bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
        <h2 className="text-2xl font-bold text-blue-900">Mes Itinéraires Sauvegardés</h2>
        <div className="flex items-center gap-2">
          <input
            ...
            className="px-3 py-1.5 bg-white/80 border border-sky-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition text-sm"
          />
          <button onClick={toggleSortOrder} title="Trier par nom" className="p-2 text-blue-600 hover:bg-blue-100 rounded-full transition">
      ...
      <div className="space-y-3 min-h-[295px]">
        {paginatedItineraries.length > 0 ? paginatedItineraries.map(it => {
          ...
          const stepsText = stepsCount > 0 ? ` (${stepsCount} étape${stepsCount > 1 ? 's' : ''})` : '';
          ...
          return (
            <div key={it.id} className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
              ...
              <div className="flex items-center space-x-1 sm:space-x-2 flex-shrink-0">
                <button onClick={() => onView(it)} title="Voir et charger" className="p-2 text-blue-600 hover:bg-blue-100 rounded-full transition"><EyeIcon className="h-5 w-5"/></button>
                <button onClick={() => onDelete(it.id)} title="Supprimer" className="p-2 text-red-600 hover:bg-red-100 rounded-full transition"><TrashIcon className="h-5 w-5"/></button>
              </div>
            </div>
          );
        }) : (
            <p className="text-center text-gray-500 pt-10">Aucun itinéraire trouvé.</p>
        )}
      </div>
